...
class ContactViewModel(friend: Friend, async: Boolean = false) : MessageNotifierViewModel(), ContactDataInterface {
    ...
    fun updateNumbersAndAddresses() {
        ...
        for (address in friend.addresses) {
            ...
            val hasLimeCapability = corePreferences.allowEndToEndEncryptedChatWithoutPresence || (friend.getPresenceModelForUriOrTel(value)?.hasCapability(Friend.Capability.LimeX3Dh) ?: false)
        ...
        for (phoneNumber in friend.phoneNumbersWithLabel) {
            ...
            val hasLimeCapability = corePreferences.allowEndToEndEncryptedChatWithoutPresence || (friend.getPresenceModelForUriOrTel(number)?.hasCapability(Friend.Capability.LimeX3Dh) ?: false)
            val secureChatAllowed = LinphoneUtils.isEndToEndEncryptedChatAvailable() && !isMe && hasLimeCapability
